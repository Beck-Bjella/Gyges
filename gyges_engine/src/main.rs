    ...
    for pos in 0..36 {
        ...
        for size in 0..=positions.len() {
            ...
            for combo in combinations {
                ...
                let key = unsafe { compress_pext(ALL_THREE_INTERCEPTS[pos as usize], temp_intercepts as u64) };

                let valid_paths_idx = unsafe { THREE_MAP.get_unchecked(pos as usize).get_unchecked(temp_intercepts as usize % 11007) };
    ...
    for pos in 0..36 {
        ...
        for size in 0..=positions.len() {
            ...
            for combo in combinations {
                ...
                let key = unsafe { compress_pext(ALL_TWO_INTERCEPTS[pos as usize], temp_intercepts as u64) };
                
                let valid_paths_idx = unsafe { TWO_MAP.get_unchecked(pos as usize).get_unchecked(temp_intercepts as usize % 29) };
                map[pos][key as usize] = *valid_paths_idx;
...
pub fn generate_combined_three_lists() {
    ...
    writeln!(file, "pub const NEW_THREE_PATH_LISTS: [ThreePathList; 8389] = [").unwrap();
    for i in 0..path_lists.len() {
        write!(file, "    ThreePathList {{ paths: [").unwrap();
        for j in 0..36 {
            write!(file, "([{}, {}, {}, {}], {}), ", path_lists[i].paths[j].0[0], path_lists[i].paths[j].0[1], path_lists[i].paths[j].0[2], path_lists[i].paths[j].0[3], path_lists[i].paths[j].1).unwrap();
        }
...
pub fn generate_combined_two_lists() {
    ...
    for i in 0..path_lists.len() {
        ...
        for j in 0..12 {
            write!(file, "([{}, {}, {}], {}), ", path_lists[i].paths[j].0[0], path_lists[i].paths[j].0[1], path_lists[i].paths[j].0[2], path_lists[i].paths[j].1).unwrap();
        }
        writeln!(file, "], count: {} }},", path_lists[i].count).unwrap();
...
pub fn generate_combined_one_lists() {
    ...
    for i in 0..path_lists.len() {
        ...
        for j in 0..4 {
            write!(file, "([{}, {}], {}), ", path_lists[i].paths[j].0[0], path_lists[i].paths[j].0[1], path_lists[i].paths[j].1).unwrap();
        }
        writeln!(file, "], count: {} }},", path_lists[i].count).unwrap();
